  ...
  useEffect(() => {
    ...
    const checkAndRefreshToken = async () => {
      try {
        ...
        console.log(
          "Refresh token exists (via getRefreshToken):",
          !!refreshToken,
        );
        ...
        if (typeof document !== "undefined") {
          ...
          if (refreshCookie) {
            ...
            console.log(
              "Refresh token found directly in document.cookie:",
              !!refreshTokenDirect,
            );
          }
        }
        ...
        if (!finalRefreshToken) {
          ...
          console.log(
            "This might be HTTP-only cookie or cookie for different domain/port",
          );
          console.log(
            "Will try to refresh anyway - cookie will be sent automatically if it exists",
          );
        }
        ...
        try {
          ...
          console.log(
            "Note: HTTP-only cookies are sent automatically even if not visible in JS",
          );
          ...
          console.warn(
            "⚠️ Token refreshed but userId not found or token not in cookies",
          );
          ...
          if (response.data && typeof response.data === "object") {
            ...
            const responseUserId =
              ...
              (data.user &&
                ...
                ((data.user as Record<string, unknown>).userId as
                  | string
                  | undefined));
